...
import (
	...
	gotflite "github.com/rai-project/go-tflite"
...
func (p *ImageClassificationPredictor) Predict(ctx context.Context, data interface{}, opts ...options.Option) error {
	if p.TraceLevel() >= tracer.FRAMEWORK_TRACE {
		...
		err := p.predictor.StartProfiling("tflite", "predict")
		if err != nil {
			log.WithError(err).WithField("framework", "tflite").Error("unable to start framework profiling")
		} else {
			defer func() {
